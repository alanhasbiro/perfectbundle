...
- [x] P2 Google AdSense — added 2026-07-22 (owner-provided `ca-pub-9391534437442090`). Site-wide loader script (`src/app/layout.tsx` via `next/script`, `afterInteractive`) + `public/ads.txt`. Two ad units wired via a shared `src/components/ad-unit.tsx`: an "auto"/full-width-responsive unit (slot `1834149356`) as a top banner on `/trending`, and a "fluid"/in-feed unit (slot `8812435332`) blended into the bundle list on `/trending`, `/popular`, **and `/quiz/results`** (owner's explicit choice — accepted the tradeoff that this sits near the affiliate Buy buttons on the highest-intent page, in exchange for more ad inventory). Verified all three placements render with real Playwright browser checks (not just raw HTML — the in-feed units only appear after a client-side Convex query resolves, so a plain `curl` can't see them). Deferred: AdSense site verification initially failed ("couldn't verify") — confirmed via direct HTTP checks that the script/ads.txt are correctly live on `perfectbundle.vercel.app` (server-rendered into raw HTML, not JS-only), so the failure is external (transient crawler issue, URL-entry mismatch in the AdSense dashboard, or — if retries keep failing — `vercel.app` shared-subdomain flakiness, which would mean revisiting the custom-domain decision).
...
- [x] P2 Warm & Bold visual redesign (2026-07-23) — replaced the pure black/white UI with a warm cream/terracotta palette + orange→pink gradient accents, applied site-wide. Spec: `docs/superpowers/specs/2026-07-23-warm-bold-visual-redesign-design.md`; plan: `docs/superpowers/plans/2026-07-23-warm-bold-visual-redesign.md`; full detail in `docs/checkpoint.md`. Executed as an 8-task subagent-driven plan; verification caught and fixed 2 real regressions before merge (Clerk v7 `appearance.variables` key names causing a build failure; `opacity-60` text failing WCAG 4.5:1 against the new palette in light mode, systemic across 15 sites — both now covered by new Vitest regression tests). Final whole-branch review caught one more gap (`ProfileForm` missed by every task's file list, still had the old flat-black button) — fixed before merge. Verified: Vitest 151/151, `tsc --noEmit` clean, Playwright 71 passed/6 skipped, Lighthouse Accessibility 100 on `/`, `/quiz`, `/quiz/results` (Performance at/above the pre-redesign baseline on all three).
...
- [ ] P2 Extend `tests/e2e/accessibility.spec.ts` to cover `/quiz/results` (and ideally `/popular`, `/b/[id]`, `/my-bundles`) with real generated `<BundleCard>` content — found via the warm-redesign final review 2026-07-23: this is exactly the page/state where a real WCAG contrast regression shipped past task-level review and was only caught by a manual one-off Lighthouse script, not a durable CI-enforced check
- [ ] P3 `--fg-muted` design token (added 2026-07-23) is defined and contrast-tested but never consumed — every "muted text" instance in the app uses `opacity-70` on `--foreground` instead (the exact mechanism that caused two separate contrast regressions as the palette changed underneath it). Either migrate real call sites to `text-fg-muted`, or drop the token if the opacity pattern is the deliberate standard
- [ ] P3 OG share images (`src/app/opengraph-image.tsx`, `src/app/b/[id]/opengraph-image.tsx`) still hardcode the old black/white palette — `next/og`'s `ImageResponse`/Satori can't read live CSS custom properties, so this needs manual hex values matching the 2026-07-23 warm palette, not a token swap
