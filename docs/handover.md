...
- **Warm & bold visual redesign (2026-07-23), fully verified, merged to `master` and pushed live.** Replaced the previous black/white theme with a warm palette (CSS custom properties `--background`/`--foreground`/`--bg-alt`/`--fg-muted`/`--accent-from`/`--accent-to`/`--accent-solid`/`--border`, light + dark) and three reusable component classes (`.btn-primary`, `.btn-secondary`, `.card-surface`), applied across the landing page/header, quiz wizard, `BundleCard`/`SaveButton` (propagates to `/quiz/results`, `/trending`, `/popular`, `/b/[id]`, `/my-bundles`), `/profiles` (page chrome + `ProfileForm`), `/my-bundles` chrome, and Clerk's auth UI. Motion/animation deliberately untouched. Spec: `docs/superpowers/specs/2026-07-23-warm-bold-visual-redesign-design.md`; plan: `docs/superpowers/plans/2026-07-23-warm-bold-visual-redesign.md`. Built via `superpowers:subagent-driven-development` in an isolated git worktree (`warm-bold-redesign` branch), fast-forward merged to `master` and pushed to origin; worktree and branch both cleaned up after merge — the `.superpowers/sdd/` task reports this session referenced no longer exist on disk (they lived only in that worktree), full detail is in `docs/checkpoint.md`'s change log instead. Final state, all fresh-verified on `master` post-merge: Vitest 151/151, `tsc --noEmit` clean, Playwright 70 passed + 1 flaky (known Gemini free-tier flakiness, not a regression — see §3) / 6 skipped, Lighthouse mobile (production build) **Accessibility 100 on `/`, `/quiz`, `/quiz/results`**, Performance `/` 95, `/quiz` 96, `/quiz/results` 94 (all at or above the pre-redesign baseline of 94/74). Three real gaps were found and fixed during verification before this merged — see §3 Gotchas: a build-blocking Clerk key-name regression, a systemic text-contrast regression, and (caught only by the final whole-branch review, after all 8 tasks' own reviews passed) `ProfileForm` being missed by every task's file list entirely. Three follow-ups from that same final review were deferred, not fixed — now tracked in `docs/tasks.md` Backlog: extending `tests/e2e/accessibility.spec.ts` to cover `/quiz/results` with real content, the unused `--fg-muted` token, and stale black/white colors in the OG share-image generators (`src/app/opengraph-image.tsx`, `src/app/b/[id]/opengraph-image.tsx` — Satori can't read CSS custom properties, so this needs manual hex updates).
...
- **E2E suite:** Playwright, 4 browser/device projects, 71 passed / 6 skipped (occasionally 70 passed + 1 flaky — see §3), reconfirmed clean after every change above.
...
- **Clerk's `appearance.variables` object key names are NOT stable across major versions — a stale name silently fails at `next build`, not at runtime.** Theming `<ClerkProvider>`'s `appearance.variables` with key names from an older Clerk major (`colorText`, `colorTextSecondary`, `colorInputBackground`, `colorInputText`) type-checked fine against nothing (they simply don't exist as valid keys), passed every runtime/E2E check (clerk-js ignores unknown keys client-side, so the UI still rendered, just unthemed on those properties), and only surfaced as a `TS2353` "object literal may only specify known properties" error on `npx tsc --noEmit`/`next build` — which is exactly the check Vercel's deploy runs. Current (`@clerk/nextjs@7.5.20`) correct names: `colorForeground`, `colorMutedForeground`, `colorInput`, `colorInputForeground`. Always re-run `tsc --noEmit` (not just Playwright) after touching Clerk's `appearance` config, and don't trust a green E2E suite alone to catch a Clerk-appearance API drift.
- **A bare Tailwind `opacity-NN` utility on plain (no explicit color class) text is a contrast liability that silently breaks whenever the underlying palette changes.** It computes a color by alpha-blending `currentColor` (here, `--foreground`) over whatever's behind it — so a value that clears WCAG AA against one palette (e.g. `opacity-60` against the old black/white theme) can fail against a new one (measured 3.7–3.8:1, need 4.5:1, against the 2026-07-23 warm palette's new backgrounds) with zero code-level signal that anything changed — the class name is identical. This is the second time this project has hit this exact failure mode (first: `opacity-50`→`opacity-60` under the old palette, 2026-07-22; second: `opacity-60`→`opacity-70` under the new one, 2026-07-23). Only a Lighthouse/axe `color-contrast` audit against a production build catches it — `tests/e2e/accessibility.spec.ts` doesn't cover `/quiz/results` with real generated content, so this shipped unnoticed twice (now tracked in `docs/tasks.md` Backlog as a follow-up, not yet fixed). `src/lib/design/contrast.ts`'s `blendHex` + `contrast.test.ts` now has a regression test for the current opacity-70 floor, but if the palette changes again, re-derive the safe opacity value (don't just copy 70) and re-run that test.
- **A fresh `git worktree add` does NOT carry over gitignored local state — `.env.local` and `.convex/` (the local Convex backend's state dir) both have to be copied in manually**, or Convex CLI calls (`npx convex run ...`, used by Playwright test setup) fail with "Failed to load deployment config" and Clerk-dependent E2E tests fail on missing env vars. Copy both from the main checkout (`cp ../../.env.local .env.local`, `cp -r ../../.convex .convex` from inside the worktree) before running any tests there — this cost two full rounds of "is this a real regression or an environment gap?" investigation in the 2026-07-23 redesign session.
- **`.next/dev/types/validator.ts` (Next.js's auto-generated dev-mode route-type file) goes stale/self-contradictory after switching between `npm run dev` and `npm run build` in the same checkout, and can make `npx tsc --noEmit` report dozens of spurious route-typing errors that have nothing to do with your actual code changes.** If `tsc --noEmit` suddenly explodes with errors only inside `.next/dev/types/`, don't debug them — `rm -rf .next` and re-run; a real type error in your own source files persists after that, a stale-artifact one disappears.
...
Per `docs/tasks.md`, next unstarted items — all P2/P3 polish, none blocking:
- Extend `tests/e2e/accessibility.spec.ts` to cover `/quiz/results` (and ideally `/popular`, `/b/[id]`, `/my-bundles`) with real generated `<BundleCard>` content — closes the durable-coverage gap noted in §3's opacity-contrast gotcha.
- Decide on `--fg-muted`: either migrate real "muted text" call sites to `text-fg-muted` instead of the `opacity-70`-on-`--foreground` pattern, or drop the unused token if the opacity pattern is the deliberate standard.
- Update `src/app/opengraph-image.tsx` / `src/app/b/[id]/opengraph-image.tsx` to the warm palette's hex values (currently still hardcoded black/white — Satori can't read live CSS custom properties).
- Occasion reminders: CRUD + daily Convex cron (T-14/T-3) → Resend email (needs owner's Resend API key for live email verification — see §2/§4).
...
(Done 2026-07-23, this session:
 • Warm & bold visual redesign — see §1 for the full summary, §3 for the three regressions
   found and fixed along the way. Merged to master and pushed to origin.)

Done earlier (still relevant context, condensed — see `docs/checkpoint.md` for full detail):
 • Curated bundles grown 5 → 23; Lighthouse a11y/perf pass with 2 bugs fixed (heading order,
   color contrast); OG/Twitter share preview images; Google AdSense wired (verification still
   pending owner retry, see §2); item swap + per-bundle regenerate; M5 authenticated E2E test;
   M3 Analytics dashboard via PostHog API; eBay Partner Network + Amazon Associates affiliate
   tags live; real eBay product photos/prices/links + Unsplash/Pexels fallback images; Etsy
   removed; Clerk Organizations/billing disabled; Google OAuth confirmed enabled; phone-number
   sign-in disabled; production Convex env vars audited (no secrets client-side).
