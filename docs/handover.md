...
- **E2E suite:** Playwright, 4 browser/device projects, 71 passed / 6 skipped (verified 2026-07-20). `npx playwright test --project=chromium` for a fast local check. New this session: `tests/e2e/auth-flow.spec.ts` covers the full signup → save → profile flow via `@clerk/testing`, closing the M5 gap that was blocked on M4.
- **Google OAuth**: confirmed already enabled on the Clerk instance (using Clerk's shared dev credentials — no setup needed while still on a dev instance). **Clerk Organizations + org billing**: found enabled with `force_organization_selection: true` (unused scaffolding default, forcing an org-selection step into every sign-in) — disabled both (owner-approved).
- **Item swap + per-bundle regenerate (DONE 2026-07-20).** "🔄 Show me another" per-item and "🔄 Regenerate" per-bundle buttons on every generated/saved/popular bundle card. Two new Convex actions (`generateBundles:swapItem`/`regenerateBundle`) reuse the existing rate limiter and Gemini/media-enrichment machinery, patching the bundle doc in place — Convex reactivity updates every page showing it automatically. Verified end-to-end with real Gemini calls: a replaced item got a genuinely fitting, media-enriched replacement; a regenerated bundle got a distinctly different theme. Closes both P1 Backlog items from M2. Plan: `docs/superpowers/plans/2026-07-20-item-swap-bundle-regenerate.md`.
...
- **`@clerk/testing`'s `clerk.signIn({ signInParams: { strategy: "password", ... } })` silently no-ops (no session, no thrown error) if `password` isn't actually configured as a first factor on the instance** — check `user_settings.attributes.password.used_for_first_factor` via the Frontend API trick above before assuming password-strategy sign-in will work in tests. Prefer `clerk.signIn({ emailAddress, page })` (the email-based overload) instead — it creates a real backend sign-in token via the ticket strategy, works regardless of configured first factors, and explicitly waits for the session to activate. A password may still be required at *user-creation* time even if unused for sign-in (`password.required` and `password.used_for_first_factor` are independent settings).
- **A hard `page.goto()` reload can leave a deeply-nested component's `useAuth()` briefly behind a shallower one's (e.g. the site header)** — unlike a real user's client-side `<Link>` navigation, which never remounts Clerk. If a component's signed-out and signed-in-but-not-yet-loaded states render identical markup (as `SaveButton`'s do), a Playwright click can silently land on the stale guest-mode branch. Use `expect(async () => { ...click...; ...assert the real effect...; }).toPass({ timeout })` to retry the interaction rather than a single click + assert.
- **In this Playwright config, `mobile-chrome` runs on the Chromium *engine*** (Pixel 7 emulation) — `test.skip(({ browserName }) => ...)` checking `browserName !== "chromium"` will NOT skip it. Use `testInfo.project.name` instead (matching `quiz-flow.spec.ts`'s existing convention) when a test should only run on the literal `"chromium"` project.
- **`testSupport.ts` seed mutations that get called repeatedly across local dev/test runs should clean up their own prior rows first** if anything ranks/limits by a shared score — `seedPopularBundle`'s fixtures all score identically via `popularityScore()`, and `listPopular()` caps at 20 with a stable tie-break, so enough accumulated runs will push a freshly seeded bundle off the visible list entirely (looks like "the query is broken" when it's really just data pollution from testing).
- **A Gemini 503 "high demand" outage can make a genuinely correct new feature look broken during manual verification.** Confirm with a direct `curl` straight to Gemini's API (bypassing Convex entirely) before assuming your own code is at fault — this happened twice in one session (once explained an E2E flake, once stalled manual `swapItem`/`regenerateBundle` verification) and both times the code was fine; Gemini just needed a few minutes.
...
(Done 2026-07-20:
 • Item swap + per-bundle regenerate — closes both P1 Backlog items from M2. See §1. Plan:
   `docs/superpowers/plans/2026-07-20-item-swap-bundle-regenerate.md`.
 ...
 • Diagnosed a "no photos" report as a stale generation-cache entry, not a regression — see
   §3 Gotchas.
 • M5 authenticated E2E test (signup → save → profile) — `tests/e2e/auth-flow.spec.ts`, via
   `@clerk/testing`. Found + fixed Clerk Organizations/billing being enabled unnecessarily,
   confirmed Google OAuth already on, plus several real test-infra bugs (data pollution,
   a hard-reload auth hydration race, a mobile-chrome skip bug). Full suite: 71 passed, 6
   skipped. Plan: `docs/superpowers/plans/2026-07-20-m5-authenticated-e2e.md`.
