...
- **Warm & bold visual redesign (2026-07-23), fully verified.** Replaced the previous black/white theme with a warm palette (CSS custom properties `--background`/`--foreground`/`--bg-alt`/`--fg-muted`/`--accent-from`/`--accent-to`/`--accent-solid`/`--border`, light + dark) and three reusable component classes (`.btn-primary`, `.btn-secondary`, `.card-surface`), applied across the landing page/header, quiz wizard, `BundleCard`/`SaveButton` (propagates to `/quiz/results`, `/trending`, `/popular`, `/b/[id]`, `/my-bundles`), `/profiles`, `/my-bundles` chrome, and Clerk's auth UI. Motion/animation deliberately untouched. Spec: `docs/superpowers/specs/2026-07-23-warm-bold-visual-redesign-design.md`; plan: `docs/superpowers/plans/2026-07-23-warm-bold-visual-redesign.md`; full verification trail (3 attempts, 2 real regressions found and fixed): `.superpowers/sdd/task-8-report.md`, `.superpowers/sdd/task-8-fix-contrast-report.md`. Final state, all fresh-verified: Vitest 151/151, `tsc --noEmit` clean, Playwright 71 passed/6 skipped, Lighthouse mobile (production build) **Accessibility 100 on `/`, `/quiz`, `/quiz/results`**, Performance `/` 95, `/quiz` 96, `/quiz/results` 94 (all at or above the pre-redesign baseline of 94/74). See §3 Gotchas for the two regressions this caught.
