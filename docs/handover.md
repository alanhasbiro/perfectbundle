...
- **Real product photos + direct retailer links — STRATEGY PIVOTED (2026-07-18).** Etsy rejected the app; Amazon/eBay expected to as well (per-retailer approval is a dead end). New plan: go through **Sovrn Commerce** — a free affiliate aggregator with day-one publisher approval whose Product API returns real photo + direct affiliate buy link + price + merchant across all major retailers (one integration = photos + buyable links + revenue). Spec: `docs/superpowers/specs/2026-07-18-product-data-and-images-design.md`.
  - **Phase 1 code SHIPPED & proven** (representative images via free **Pexels**, env-gated `PEXELS_API_KEY`, cached, labelled, best-effort). Plan: `docs/superpowers/plans/2026-07-18-phase1-representative-images.md`. **BLOCKED live on a bad key:** the `PEXELS_API_KEY` currently in `.env.local` is invalid — Pexels returns "Invalid API key" for every fresh query (a CDN-cached "coffee" query masked it); it's 32 chars vs Pexels' ~56, so the wrong value was pasted. **Owner:** re-copy the full key from pexels.com/api → replace `PEXELS_API_KEY` → tell Claude to re-sync Convex env + verify live.
